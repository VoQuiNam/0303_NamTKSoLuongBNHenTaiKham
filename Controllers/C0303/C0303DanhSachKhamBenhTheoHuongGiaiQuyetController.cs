...
{
    [Route("danh_sach_kham_benh_theo_huong_giai_quyet")]
    public class C0303DanhSachKhamBenhTheoHuongGiaiQuyetController : Controller
    {
        //private string _maChucNang = "/danh_sach_kham_benh_theo_huong_giai_quyet";
        ...
        public C0303DanhSachKhamBenhTheoHuongGiaiQuyetController(
            Context0303 localDb,
        ...
        public IActionResult V0303DanhSachKhamBenhTheoHuongGiaiQuyetPage()
        {
            ...
            return View("~/Views/V0303/V0303DanhSachKhamBenhTheoHuongGiaiQuyet/V0303DanhSachKhamBenhTheoHuongGiaiQuyetPage.cshtml");
        }

        [HttpGet("giai-quyet/all")]
        public async Task<List<M0303DM_GiaiQuyet>> GetDM_GiaiQuyet()
        {
            try
            {
                var dmGiaiQuyet = await _localDb.Set<M0303DM_GiaiQuyet>()
                .FromSqlRaw(@"  SELECT ID AS ID, TenGiaiQuyet AS TenGiaiQuyet FROM [dbo].[DM_GiaiQuyet]")
                .Select(gq => new M0303DM_GiaiQuyet
                {
                    id = gq.id,
                    TenGiaiQuyet = gq.TenGiaiQuyet ?? ""
                })
                .ToListAsync();
        ...
        public async Task<IActionResult> FilterByDay(string tuNgay, string denNgay, int idChiNhanh, int IdGiaiQuyet)
        {
            ...
            var data = await _localDb.Set<M0303DanhSachKhamBenhTheoHuongGiaiQuyetSTO>()
                .FromSqlRaw(@"EXEC S0305_DSKhamBenhTheoHSQ @TuNgay, @DenNgay, @IDCN, @IdGiaiQuyet",
                    ...
                    new SqlParameter("@IdGiaiQuyet",IdGiaiQuyet),
        ...
        [HttpGet("export/pdf")]
        public async Task<IActionResult> ExportToPDF([FromQuery] DateTime? tuNgay, [FromQuery] DateTime? denNgay, [FromQuery] int? idChiNhanh, [FromQuery] string? idnv = null, [FromQuery] int? IdGiaiQuyet = 0)
        {
            try
            {

                var data = await GetBNHenKhamAsync(tuNgay, denNgay, idChiNhanh, idnv, IdGiaiQuyet);
                ...
                var document = new P0303DanhSachKhamBenhTheoHuongGiaiQuyet(data, tuNgay, denNgay, logoPath, thongTinDoanhNghiep, idnv, IdGiaiQuyet);
    ...
    [FromQuery] int? IdGiaiQuyet = 0) // 🆕 THÊM IDGIAIQUYET
        {
            try
            {
                var data = await GetBNHenKhamAsync(tuNgay, denNgay, idcn, idnv, IdGiaiQuyet); // 🆕 TRUYỀN IDGIAIQUYET
                if (!data.Any())
                ...
                var ws = workbook.Worksheets.Add("DS KHÁM BỆNH THEO GQ");
                ...
                // ====== TIÊU ĐỀ ======
                ws.Range("D6:H6").Merge().Value = "DANH SÁCH KHÁM BỆNH THEO HƯỚNG GIẢI QUYẾT"; // 🆕 SỬA TIÊU ĐỀ
                ...
                // 🆕 HIỂN THỊ THÔNG TIN GIẢI QUYẾT NẾU CÓ
                string giaiQuyetInfo = "";
                if (IdGiaiQuyet.HasValue && IdGiaiQuyet.Value > 0)
                {
                    var selectedGiaiQuyet = data.FirstOrDefault()?.TenGiaiQuyet;
                    if (!string.IsNullOrEmpty(selectedGiaiQuyet))
                    {
                        giaiQuyetInfo = $" - Hướng giải quyết: {selectedGiaiQuyet}";
                    }
                }

                string thoiGianThongKe = tuNgay.HasValue && denNgay.HasValue
                    ? $"Từ ngày {tuNgay:dd-MM-yyyy} đến ngày {denNgay:dd-MM-yyyy}{giaiQuyetInfo}"
                    : $"Toàn bộ thời gian{giaiQuyetInfo}";
                ...
                // ====== NHÓM DỮ LIỆU THEO HƯỚNG GIẢI QUYẾT ====== 🆕 SỬA THÀNH GIẢI QUYẾT
                var groupedByGiaiQuyet = data
                    .GroupBy(x => x.TenGiaiQuyet ?? "Không rõ giải quyết")
                    .ToDictionary(g => g.Key, g => g.ToList());

                row = 10;
                int stt = 1;

                // ====== DUYỆT QUA TỪNG NHÓM GIẢI QUYẾT ====== 🆕 SỬA THÀNH GIẢI QUYẾT
                foreach (var giaiQuyetGroup in groupedByGiaiQuyet)
                {
                    string giaiQuyet = giaiQuyetGroup.Key;
                    var patients = giaiQuyetGroup.Value;

                    // Tính tổng cho giải quyết này
                    int subTotalSoLuong = patients.Sum(x => x.SoLuong ?? 0);
                    int subTotalBHYT = patients.Count(x => x.BHYT == true); // 🆕 SỬA ĐIỀU KIỆN BHYT

                    // Dòng header cho giải quyết
                    ws.Cell(row, 1).Value = $"- {giaiQuyet}";
                    ...
                    foreach (var item in patients)
                    {
                        ...
                        ws.Cell(row, 4).Value = item.TenBN ?? ""; // 🆕 SỬA PROPERTY

                        ws.Cell(row, 5).Value = item.NgaySinh?.ToString("dd-MM-yyyy") ?? "";
                        ws.Cell(row, 5).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;

                        ws.Cell(row, 6).Value = item.BHYT == true ? "X" : ""; // 🆕 SỬA ĐIỀU KIỆN BHYT
                        ws.Cell(row, 6).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
                ...
                return File(stream.ToArray(),
                    ...
                    $"DanhSachKhamBenhTheoHuongGiaiQuyet_{DateTime.Now:yyyyMMddHHmmss}.xlsx"); // 🆕 SỬA TÊN FILE
            }
            catch (Exception ex)
            {
        ...
        [NonAction]
        public async Task<List<M0303DanhSachKhamBenhTheoHuongGiaiQuyetSTO>> GetBNHenKhamAsync(DateTime? tuNgay, DateTime? denNgay, int? idChiNhanh, string? idnv = null, int? IdGiaiQuyet = 0)
        {
            ...
            return await _localDb.M0303DanhSachKhamBenhTheoHuongGiaiQuyetSTOs
                .FromSqlInterpolated($"EXEC S0305_DSKhamBenhTheoHSQ @TuNgay = {tuNgayStr}, @DenNgay = {denNgayStr}, @IDCN = {idCN}, @IdGiaiQuyet = {IdGiaiQuyet}")
                .ToListAsync();
        }

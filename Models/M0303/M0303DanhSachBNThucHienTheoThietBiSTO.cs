namespace Nam_ThongKeSoLuongBNHenTaiKham.Models.M0303
{
    public class M0303DanhSachBNThucHienTheoThietBiSTO
    {
        //public string? MaYT { get; set; }
        //public string? SoHS { get; set; }
        //public string? SoBA { get; set; }
        //public string? ICD { get; set; }
        //public string? HoTen { get; set; }
        //public string? GioiTinh { get; set; }
        //public string? SoBHYT { get; set; }
        //public string? KCBBD { get; set; }
        //public bool? DT { get; set; }
        //public string? DoiTuong { get; set; }
        //public string? TinhTrang { get; set; }
        //public string? NoiChiDinh { get; set; }
        //public string? BacSi { get; set; }
        //public long? IdDichVuKyThuat { get; set; }
        //public long? IdNhomDichVu { get; set; }
        //public int? SoLuong { get; set; }
        //public DateTime? NgayYC { get; set; }
        //public DateTime? NgayTH { get; set; }
        //public string? QuyenSo { get; set; }
        //public string? SoBL { get; set; }
        //public string? ChungTu { get; set; }
        //public string? TenThietBi { get; set; }
        //public decimal? DoanhThu { get; set; }
        //public decimal? BaoHiem { get; set; }
        //public decimal? DaThanhToan { get; set; }
        //public decimal? ChuaThanhToan { get; set; }
        //public bool? HuyHoan { get; set; }
        //public bool? TrangThaiThanhToan { get; set; }

        public string? MaYT { get; set; }
        public string? SoHS { get; set; }
        public string? SoBA { get; set; }
        public string? ICD { get; set; }
        public string? HoTen { get; set; }
        public string? TenGioiTinh { get; set; }
        public string? SoBHYT { get; set; }
        public string? KCBBD { get; set; }
        public string? DT { get; set; }               // số điện thoại
        public string? DoiTuong { get; set; }
        public string? TinhTrang { get; set; }
        public string? NoiChiDinh { get; set; }
        public string? BacSi { get; set; }
        public long? IDDVKT { get; set; }
        public string? TenDichVuKyThuat { get; set; } // tên dịch vụ, không phải ID
        public long? IDNDVKT { get; set; }
        public string? TenNhomDichVu { get; set; }    // tên nhóm dịch vụ
        public int? SoLuong { get; set; }
        public DateTime? NgayYC { get; set; }
        public DateTime? NgayTH { get; set; }
        public string? QuyenSo { get; set; }
        public string? SoBL { get; set; }
        public string? ChungTu { get; set; }
        public string? TenThietBi { get; set; }
        public decimal? DoanhThu { get; set; }
        public decimal? BaoHiem { get; set; }          // để string cho khớp STO
        public decimal? DaThanhToan { get; set; }
        public decimal? ChuaThanhToan { get; set; }    // có thể là số hoặc rỗng
        public string? HuyHoan { get; set; }          // 'x' hoặc ''
        public string? TrangThaiThanhToan { get; set; } // 'x' hoặc ''
    }

}
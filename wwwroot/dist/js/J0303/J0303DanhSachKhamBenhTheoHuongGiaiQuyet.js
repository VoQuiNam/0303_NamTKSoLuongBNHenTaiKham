...
let tomSelectNhomGiaiQuyet = null;

function initTomSelectNhomGiaiQuyet() {
    const selectElement = document.getElementById('nhomGiaiQuyetSelect');

    if (tomSelectNhomGiaiQuyet) {
        tomSelectNhomGiaiQuyet.destroy();
    }
    ...
    tomSelectNhomGiaiQuyet = new TomSelect(selectElement, {
        valueField: 'id',
        labelField: 'tenGiaiQuyet',
        searchField: ['tenGiaiQuyet', 'vietTat'], // THÊM 'vietTat' vào searchField
        placeholder: 'Chọn nhóm giải quyết',
        ...
        render: {
            option: function (data, escape) {
                const tenGiaiQuyet = data.tenGiaiQuyet || '';
                const vietTat = data.vietTat || (tenGiaiQuyet === 'Tất cả' ? '' : generateAbbreviation(tenGiaiQuyet));
                return `
                <div class="d-flex justify-content-between">
                    <span>${escape(tenGiaiQuyet)}</span>
                    <span class="text-muted">${vietTat ? '[' + escape(vietTat) + ']' : ''}</span>
                </div>
            `;
            },
            item: function (data, escape) {
                return `<div>${escape(data.tenGiaiQuyet || '')}</div>`;
            }
        },
        ...
        onChange: function (value) {
            ...
            if (selectedItem) {
                const tenGiaiQuyet = selectedItem.tenGiaiQuyet || '';
                const vietTat = selectedItem.vietTat || (tenGiaiQuyet === 'Tất cả' ? '' : generateAbbreviation(tenGiaiQuyet));

                document.getElementById('nhomGiaiQuyetIdHidden').value = selectedItem.id || 0;
                document.getElementById('nhomGiaiQuyetTenGiaiQuyetHidden').value = tenGiaiQuyet;
                document.getElementById('nhomGiaiQuyetKetThucHSHidden').value = selectedItem.ketThucHS || '';
            } else {
                document.getElementById('nhomGiaiQuyetIdHidden').value = 0;
                document.getElementById('nhomGiaiQuyetTenGiaiQuyetHidden').value = 'Tất cả';
                document.getElementById('nhomGiaiQuyetKetThucHSHidden').value = '';
            }
        }
    });

    tomSelectNhomGiaiQuyet.on('type', function (str) { });

    setTimeout(() => {
        tomSelectNhomGiaiQuyet.close();
        tomSelectNhomGiaiQuyet.blur();
    }, 10);
}

function updateTomSelectNhomGiaiQuyetData(data) {
    if (tomSelectNhomGiaiQuyet) {
        // Xóa các option cũ (trừ option "Tất cả")
        const currentOptions = tomSelectNhomGiaiQuyet.options;
        Object.keys(currentOptions).forEach(key => {
            if (key !== '0') {
                tomSelectNhomGiaiQuyet.removeOption(key);
            }
        });

        // Thêm dữ liệu mới
        if (data && Array.isArray(data)) {
            tomSelectNhomGiaiQuyet.addOptions(data);
            tomSelectNhomGiaiQuyet.refreshOptions();
        }
    }
}

async function loadNhomGiaiQuyet() {
    try {
        console.log('Đang tải danh sách nhóm giải quyết...');
        const response = await fetch('/danh_sach_kham_benh_theo_huong_giai_quyet/giai-quyet/all');

        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }

        const giaiQuyetData = await response.json();
        console.log('Dữ liệu nhóm giải quyết:', giaiQuyetData);

        // Kiểm tra và làm sạch dữ liệu
        if (!giaiQuyetData || !Array.isArray(giaiQuyetData)) {
            console.warn('Dữ liệu nhóm giải quyết không hợp lệ:', giaiQuyetData);
            return;
        }

        // Thêm option "Tất cả" vào đầu danh sách và tạo từ viết tắt
        const dataWithDefault = [
            {
                id: 0,
                tenGiaiQuyet: 'Tất cả',
                ketThucHS: false,
                vietTat: '' // Thêm trường vietTat
            },
            ...giaiQuyetData.map(item => ({
                ...item,
                vietTat: generateAbbreviation(item.tenGiaiQuyet) // Tạo từ viết tắt cho mỗi item
            }))
        ];

        // Sắp xếp dữ liệu theo tên (trừ option "Tất cả")
        const sortedData = [
            dataWithDefault[0], // Giữ "Tất cả" ở đầu
            ...dataWithDefault.slice(1).sort((a, b) => {
                const tenA = a.tenGiaiQuyet || '';
                const tenB = b.tenGiaiQuyet || '';
        ...
        // Khởi tạo TomSelect
        initTomSelectNhomGiaiQuyet();

        // Cập nhật dữ liệu vào TomSelect
        updateTomSelectNhomGiaiQuyetData(sortedData);

    } catch (err) {
        console.error("Lỗi load nhóm giải quyết:", err);
        toastr.error("Không thể tải danh sách nhóm giải quyết");
    }
}
...
function handleFilter() {
    $('.btnFilterKhamBenhTheoBacSi').off('click').on('click', function (e) {
        ...
        setTimeout(function () {
            try {
                let tuNgayRaw = $('#tuNgayDesktop').val() || $('#tuNgayMobile').val();
                let denNgayRaw = $('#denNgayDesktop').val() || $('#denNgayMobile').val();
                const idGiaiQuyet = document.getElementById('nhomGiaiQuyetIdHidden').value || 0;
                ...
                $.ajax({
                    url: '/danh_sach_kham_benh_theo_huong_giai_quyet/tk/FilterByDay',
                    type: 'POST',
                    data: {
                        tuNgay,
                        denNgay,
                        idChiNhanh: window._idcn || 0,
                        IdGiaiQuyet: idGiaiQuyet // THÊM ID GIAI QUYET VÀO REQUEST
                    },
                    beforeSend: function () {
                        showLoading();
                    },
...
function renderTable() {
    ...
    setTimeout(() => {
        try {
            ...
            // 🆕 NHÓM TOÀN BỘ DỮ LIỆU ĐÃ SẮP XẾP THEO TEN GIAI QUYET
            let groupedByGiaiQuyet = {};
            sortedData.forEach(item => {
                let giaiQuyet = item.tenGiaiQuyet || "Không rõ giải quyết";
                if (!groupedByGiaiQuyet[giaiQuyet]) groupedByGiaiQuyet[giaiQuyet] = [];
                groupedByGiaiQuyet[giaiQuyet].push(item);
            });

            // 🆕 TẠO MẢNG DỮ LIỆU ĐÃ ĐƯỢC NHÓM THEO TEN GIAI QUYET VÀ SẮP XẾP
            let groupedData = [];
            Object.keys(groupedByGiaiQuyet).forEach(giaiQuyet => {
                groupedData = groupedData.concat(groupedByGiaiQuyet[giaiQuyet]);
            });
            ...
            // 🆕 XÁC ĐỊNH GIAI QUYET NÀO CÓ TRONG TRANG HIỆN TẠI
            let currentGiaiQuyet = null;
            let displayedGiaiQuyet = new Set();

            pageData.forEach((item, index) => {
                const giaiQuyet = item.tenGiaiQuyet || "Không rõ giải quyết";

                // 🆕 NẾU GẶP GIAI QUYET MỚI, THÊM HEADER
                if (giaiQuyet !== currentGiaiQuyet && !displayedGiaiQuyet.has(giaiQuyet)) {
                    const allPatientsOfGiaiQuyet = groupedByGiaiQuyet[giaiQuyet];
                    const subTotalSoLuong = allPatientsOfGiaiQuyet.reduce((sum, item) => sum + (Number(item.soLuong) || 0), 0);
                    const subTotalBHYT = allPatientsOfGiaiQuyet.filter(item => item.bhyt).length;

                    html += `
                        <tr class="giaiquyet-header-row" style="font-weight: bold;">
                            <td class="text-start fw-bold" colspan="5">- ${giaiQuyet}</td>
                    ...
                    currentGiaiQuyet = giaiQuyet;
                    displayedGiaiQuyet.add(giaiQuyet);
                }
                ...
                row += `<td class="text-start">${item.tenBN || ''}</td>`;
...
function exportPDFHandler(btn, viewType) {
    ...
    let url = "/danh_sach_kham_benh_theo_huong_giai_quyet/export/pdf?";
    ...
    if (idGiaiQuyet) url += `IdGiaiQuyet=${idGiaiQuyet}&`; // 🆕 THÊM ID GIAI QUYET
    if (idnv) url += `idnv=${encodeURIComponent(idnv)}`;


    console.log('nhan vien: ', idnv);
...
async function handleExportExcel() {
    ...
    btn.addEventListener("click", async function () {
        ...
        try {
            ...
            const response = await fetch(`/danh_sach_kham_benh_theo_huong_giai_quyet/check-and-export?tuNgay=${tuNgay}&denNgay=${denNgay}&idcn=${idChiNhanh}&idnv=${idnv}&IdGiaiQuyet=${idGiaiQuyet}`);
            ...
            a.href = url;
            a.download = "DanhSachKhamBenhTheoHuongGiaiQuyet.xlsx";
